...
/// @file
/// @brief Differential drive kinematics class source.
/// Adapted from https://github.com/ngmor/turtlebot3-ekf-slam/blob/main/turtlelib/src/diff_drive.cpp
...
DiffDrive::DiffDrive(
  double wheel_track,
  double wheel_radius,
  const Config& start_config
):
wheel_track_{wheel_track},
wheel_radius_{wheel_radius},
config_init_{start_config},
...
const DiffDrive::Config& DiffDrive::config() const { return config_; }

void DiffDrive::set_config(const Config& new_config) { config_ = new_config; }

void DiffDrive::set_pose(const Transform2D& new_pose) { config_.pose = new_pose; }

void DiffDrive::set_wheel_pos(const Wheel& new_wheel_pos) { config_.wheel_pos = new_wheel_pos; }
...
const DiffDrive::Config& DiffDrive::update_config(const Wheel& new_wheel_pos)
{
  //Calculate produced body twist
  Twist2D body_twist = get_body_twist(new_wheel_pos);

  //integrate twist and compose it with the current pose transformation
  //Twbp = Twb * Tbbp
  config_.pose *= integrate_twist(body_twist);    //doc/Kinematics.pdf Eq 3.1-3.7
